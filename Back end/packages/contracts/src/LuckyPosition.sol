...
contract LuckyPosition is UUPSUpgradeable, AutomationCompatibleInterface, VRFConsumerBaseV2PlusUpgradeable, PausableUpgradeable {
    ...
    event GameCreated(uint256 indexed gameId, uint256 ticketPrice, uint256 startTime, uint256 endTime);
    event PlayerJoined(uint256 indexed gameId, uint256 indexed playerId, address indexed player, uint256 joinedAt);
    event GameEnded(uint256 indexed gameId, address indexed winner, uint256 totalReward);
    event GamePaused(uint256 indexed gameId, uint256 pausedAt);
    event GameResumed(uint256 indexed gameId, uint256 resumedAt);
    ...
    event RefundClaimed(address indexed player, uint256 amount, uint256[] gameIds, uint256 refundedAt);
    ...
    function pauseGame() external onlyOwner {
        ...
        emit GamePaused(gameCounter, block.timestamp);
    }
    ...
    function resumeGame() external onlyOwner {
        ...
        emit GameResumed(gameCounter, block.timestamp);
    }
    ...
    function createGame(
        ...
        emit GameCreated(gameCounter, _ticketPrice, block.timestamp, endTime );
    }
    ...
    function joinGame() external whenNotPaused {
        ...
        uint256 playerId = playerTickets.length;
        ...
        emit PlayerJoined(gameCounter, playerId, msg.sender, block.timestamp);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) internal override {
        ...
        emit GameEnded(gameCounter, winner, reward);
    ...
    function claimRefund(uint256[] calldata _gameIds) external {
        ...
        emit RefundClaimed(msg.sender, totalRefund, _gameIds, block.timestamp);
    }
